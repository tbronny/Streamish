        ...
        public Video GetVideoByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"
                        SELECT v.Title, v.Description, v.Url, v.DateCreated, v.UserProfileId As VideoUserProfileId,
                            up.Name as UserProfileName, up.Email, up.DateCreated AS UserProfileDateCreated,
                            up.ImageUrl AS UserProfileImageUrl,
                            c.Id AS CommentId, cu.Name as CommenterName, c.Message, c.UserProfileId AS CommentUserProfileId
                        FROM Video v
                        JOIN UserProfile up ON v.UserProfileId = up.Id
                        LEFT JOIN Comment c on c.VideoId = v.id
                        ...
                        if (DbUtils.IsNotDbNull(reader, "CommentId"))
                        {
                            video.Comments.Add(new Comment()
                            {
                                ...
                                UserProfile = new UserProfile()
                                {
                                    ...
                                    Name = DbUtils.GetString(reader, "CommenterName")
                                }
                            });
                        }
                    }
